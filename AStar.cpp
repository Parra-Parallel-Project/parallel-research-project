...
void AStar::unthreadedAStar(int start, int stop, WeightedGraph& graph) {
...
void AStar::threadedAStar(int start, int stop, WeightedGraph& graph, int numberThreads) {
    const int INF = std::numeric_limits<int>::max();
...
void AStar::subThreadAStar(WeightedGraph& graph, vector<int>& dist, vector<bool>& visited, int start, int stop) {
    const int INF = std::numeric_limits<int>::max();
    ...
    while (true) {
        ...
        for (std::list<edge>::iterator it = graph.getEdges(u)->begin(); it != graph.getEdges(u)->end(); ++it) {
            ...
            if (!visited[v] && dist[u] + weight < dist[v]) {
                dist[v] = dist[u] + weight;
		dist[v] = dist[v] + std::abs(v - stop); //calculates distance to find ideal path
            }
        }
    }
}